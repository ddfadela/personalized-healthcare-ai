...
\hline
\cite{article_10} & Heart Disease & UCI Heart Disease Dataset via Kaggle & Stratified 80/20 train-test split , robust scaling & Deep Q-Network (DQN) & Accuracy: 98.41\%, MSE: 0.0001 \\
\hline
\cite{article_11} & Skin Cancer & HAM10000 dataset (10,000+ dermatoscopic images) & Feature extraction via CNN,  normalization  &  RL (Deep Q-learning), SL (CNN)  &  \\
\hline
\cite{article_12} & Brain Tumor Localization & BraTS 2014 (T1-weighted contrast-enhanced MRI, 60 images total) & Grid-based image division, 60x60 pixel block navigation & Deep Q-Network (DQN) & 70\% accuracy vs 11\% for SL;\\
\hline
\cite{article_13} & Sepsis Treatment & 500 simulated patients using IIRABM &  & Deep Reinforcement Learning  &  \\
\hline
\cite{article_14} & Diabetes Treatment & Custom benchmark dataset  &  Feature extraction & Deep RL  & 117-153\% improvement in single-step rewards, 40.5\% increase in precision, 63\% increase in recall \\
\hline
\cite{article_15} & Multi-class Clinical Classification & Real-world clinical datasets & Feature selection, normalization & Combined dueling and double deep Q-learning &  \\
\hline
\cite{article_16} & Sepsis and Acute Hypotension & Real-world datasets  &  & Transformer-based Deep Attention Q-Network &  better expected rewards than clinician policies \\
\hline
\cite{article_17} & Oropharyngeal squamous cell carcinoma & 536 patient records (MD Anderson, 2005–2013) & Random split (75\% train, 25\% test); radiomics used in select training runs & Deep Q-Learning & Achieved 87.35\% mean accuracy; improved predicted survival by 3.73\% and reduced dysphagia by 0.75\% \\
\hline
\cite{article_18} & Type 2 diabetes & 16,665 EHRs from NYU Langone (2009–2017) & 60\% train, 40\% test; excluded non-longitudinal or emergency visits & RL models & High concordance with clinicians; better outcomes in disagreement cases (86.1\% for glycemia, 98.4\% for CVD) \\
\hline
\cite{article_19} & Cancer therapy personalization & 1080 RNA-seq patients, 223 drugs, TCGA + GDSC & Filtered samples by PubChem ID & PPORank (DRL), actor-critic with PPO; ranks treatments by optimizing NDCG & next to 0.8 NDCG \\
\hline
\cite{article_20} & COVID-19 diagnosis using CT images & 1,065 CT images & ROI extraction, resized to 299x299, RGB conversion  & Modified Inception model (M-inception), fine-tuned with transfer learning & 89.5\% accuracy on internal validation; 85.2\% on difficult false-negative cases \\
\hline
